...
CAMERA_MAPPING = {
    'F_MIDLONGRANGECAM_CL': 'F_MIDLONGRANGECAM_CL',
...
class CameraData:
    ...
    def __init__(self, front_img: np.array, back_img: np.array, left_img: np.array,
        ...
        """
        self.front_camera = CameraDataItem('front_cam', front_img, camera_params_by_sensor['F_MIDLONGRANGECAM_CL'])
    ...
    if os.path.exists(cali_json_path):
        ...
        for sensor, params in camera_params_json.items():
            if sensor in CAMERA_MAPPING and 'RT_sensor_from_body' in params:
                ...
                yaw_pitch_roll = np.array(params['yaw_pitch_roll_deg'])
                camera_params = CameraParams(intrinsic, extrinsic, dist, yaw_pitch_roll, model)
