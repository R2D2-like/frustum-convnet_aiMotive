...
class ProviderDataset(Dataset):
        ...
        if from_rgb_detection:
            ...
            if extend_from_det:
                ...
                with open(extend_det_file, 'rb') as fp:
                    ...
                    self.size_list.extend(pickle.load(fp)) # l, w, h
    ...
    def __getitem__(self, index):
        ...
        size_class = self.category_info.CLASSES.index(cls_type) # 0: car, 1: pedestrian, 2: cyclist
        ...
        box = self.box2d_list[index] # [x1, y1, x2, y2]
        ...
        # ------------------------------ LABELS ----------------------------
        seg = self.label_list[index].astype(np.int64) # 0: unannotated, 1: in, -1: out
        seg = seg[choice]
    ...
    def generate_labels(self, center, dimension, angle, ref_xyz, P):
        ...
        return labels # 1: inside, -1: outside, 0: border

    def generate_ref(self, box, P):

        s1, s2, s3, s4 = cfg.DATA.STRIDE #STRIDE: (0.25, 0.5, 1.0, 2.0)
        max_depth = cfg.DATA.MAX_DEPTH #MAX_DEPTH: 200
        print('max_depth', max_depth)

        z1 = np.arange(0, max_depth, s1) + s1 / 2. # frustumの中心（奥行き方向）
        z2 = np.arange(0, max_depth, s2) + s2 / 2.
        z3 = np.arange(0, max_depth, s3) + s3 / 2.
        z4 = np.arange(0, max_depth, s4) + s4 / 2.

        cx, cy = (box[0] + box[2]) / 2., (box[1] + box[3]) / 2.,  # boxの中心

        xyz1 = np.zeros((len(z1), 3))
        xyz1[:, 0] = cx
        xyz1[:, 1] = cy
        xyz1[:, 2] = z1
        xyz1_rect = project_image_to_rect(xyz1, P) # image2camera
