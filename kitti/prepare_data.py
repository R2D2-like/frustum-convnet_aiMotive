...
DATA_DIR = 'data/aimotive-kitti_format' #'data/kitti'
...
def extract_frustum_det_data(idx_filename, split, output_filename, det_filename,
    ...
    dataset = kitti_object(os.path.join(ROOT_DIR, DATA_DIR), split)
    ...
    for data_idx in data_idx_list:
        ...
        for obj_idx in range(len(det_objects)):
            ...
            if cur_obj['type'] not in type_whitelist:
                print('type not in whitelist', cur_obj['type'])
                continue
            ...
            if max_overlap < thresh:
                print('max overlap < thresh', max_overlap)
                continue
...
def extract_frustum_data(idx_filename, split, output_filename,
    ...
    '''
    print(idx_filename) #/root/frustum-convnet/kitti/image_sets/train.txt
    dataset = kitti_object(os.path.join(ROOT_DIR, DATA_DIR), split)
    data_idx_list = [int(line.rstrip()) for line in open(idx_filename)]
    ...
    for data_idx in data_idx_list:
        ...
        pc_velo = dataset.get_lidar(data_idx) # lidar points in lidar coord
        pc_rect = np.zeros_like(pc_velo)
        pc_rect[:, 0:3] = calib.project_velo_to_rect(pc_velo[:, 0:3])
        pc_rect[:, 3] = pc_velo[:, 3]
        # print('pc_rect', pc_rect) # (n, 4)
        img = dataset.get_image(data_idx)
        img_height, img_width, img_channel = img.shape
        # print('Image shape: ', img.shape) #704, 1280, 3
        _, pc_image_coord, img_fov_inds = get_lidar_in_image_fov(pc_velo[:, 0:3],
                                                                 calib, 0, 0, img_width, img_height, True)
...
def get_box3d_dim_statistics(idx_filename):
    ...
    dataset = kitti_object(os.path.join(ROOT_DIR, DATA_DIR))
    dimension_list = []
    type_list = []
    data_idx_list = [int(line.rstrip()) for line in open(idx_filename)]
    for data_idx in data_idx_list:
...
def read_det_file(det_filename):
    ...
    # det_id2str = {1: 'Pedestrian', 2: 'CAR', 3: 'Cyclist'} # default definition in rgb_detection_train/val.txt by 
    det_id2str = {1: 'CAR', 2: 'TRUCK', 3: 'BUS', 4: 'RIDER', 5: 'MOTORCYCLE', 6: 'PEDESTRIAN', 7: 'BICYCLE', 8: 'TRAIN'} # default definition in rgb_detection_train/val.txt
    # det_id2str = {1: 'CAR', 2: 'VAN', 3: 'BUS', 4: 'RIDER', 5: 'MOTORCYCLE', 6: 'PEDESTRIAN', 7: 'BICYCLE', 8: 'TRAIN'} # default definition in rgb_detection_train/val.txt
    id_list = []
    type_list = []
    prob_list = []
    box2d_list = []
...
def extract_frustum_data_rgb_detection(det_filename, split, output_filename,
    ...
    Output:
        None (will write a .pickle file to the disk)
    '''
    dataset = kitti_object(os.path.join(ROOT_DIR, DATA_DIR), split=split)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_vis_rgb_detection', action='store_true',
                        help='Generate vis frustum data with RGB detection 2D boxes')
    ...
    if args.car_only:
        ...
        print('Including all object types.')
        # type_whitelist = ['CAR', 'Pedestrian', 'Cyclist'] # kitti
        type_whitelist = ['CAR', 'TRUCK', 'BUS', 'RIDER', 'MOTORCYCLE', 'PEDESTRIAN', 'BICYCLE','TRAIN'] #aimotive
    ...
    if args.gen_vis_rgb_detection:
        extract_frustum_data_rgb_detection(
            os.path.join(BASE_DIR, 'rgb_detections/rgb_detection_vis.txt'),
            'training',
            os.path.join(save_dir, output_prefix + 'vis_rgb_detection.pickle'),
            type_whitelist=type_whitelist)
