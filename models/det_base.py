...
from datasets.dataset_info import AIMOTIVECategory
...
class PointNetModule(nn.Module):
    def __init__(self, Infea, mlp, dist, nsample, use_xyz=True, use_feature=True): # dist = HEIGHT_HALF = u
    ...
    def forward(self, pc, feat, new_pc=None):
        ...
        if self.use_xyz:
            ...
            ).view(batch_size, 3, npoint, k) # ここでは、pcの点を取得している
...
class PointNetDet(nn.Module):
    ...
    def forward(self,
        ...
        object_point_cloud_xyz = point_cloud[:, :3, :].contiguous()  # b, 3, n
        if point_cloud.shape[1] > 3:
            object_point_cloud_i = point_cloud[:, [3], :].contiguous() # intensity
        else:
