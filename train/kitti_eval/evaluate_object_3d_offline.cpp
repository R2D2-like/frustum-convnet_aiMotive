...
enum CLASSES{CAR=0, PEDESTRIAN=1, TRUCK=2, BUS=3, RIDER=4, MOTORCYCLE=5, BICYCLE=6, TRAIN=7};
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
  ...
  for(int32_t i=0;i<gt.size(); i++){
    ...
    else if(!strcasecmp(CLASS_NAMES[current_class].c_str(), "CAR") && !strcasecmp("BUS", gt[i].box.type.c_str()))
      valid_class = 0;
    else if(!strcasecmp(CLASS_NAMES[current_class].c_str(), "Car") && !strcasecmp("TRUCK", gt[i].box.type.c_str()))
      valid_class = 0;
