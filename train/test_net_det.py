...
def write_detection_results(output_dir, det_results):
    ...
    # there is a TXT file
    idx_path = 'kitti/image_sets/%s.txt' % cfg.TEST.DATASET # val.txt
...
def write_detection_results_nms(output_dir, det_results, threshold):
    ...
    for data_idx in nms_results:
        img = cv2.imread('data/aimotive-kitti_format/training/image_2/%06d.png' % data_idx)
        for class_type in det_results[data_idx]:
            for output in det_results[data_idx][class_type]:
                x1, y1, x2, y2, tx, ty, tz, h, w, l, ry, score = output
                cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
                cv2.putText(img, class_type, (int(x1), int(y1)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        # save the image
        if not os.path.exists('output/result/nms_viz2'):
            os.makedirs('output/result/nms_viz2')
        cv2.imwrite('output/result/nms_viz2/%06d.png' % data_idx, img)
...
def evaluate_py_wrapper(output_dir, async_eval=False):
    # official evaluation
    gt_dir = 'data/aimotive-kitti_format/training/label_2/'
    ...
    for data_idx in det_results:
        ...
        for class_type in det_results[data_idx]:
            for output in det_results[data_idx][class_type]:
                ...
                cv2.putText(img, class_type, (int(x1), int(y1)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        # save the image
        if not os.path.exists('output/result/viz'):
            os.makedirs('output/result/viz')
        cv2.imwrite('output/result/viz/%06d.png' % data_idx, img)
    '''


    # Write detection results for KITTI evaluation

    if cfg.TEST.METHOD == 'nms': # this
